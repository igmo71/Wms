...
public record ServiceError(ServiceErrorType Type, string? Message)
{
    ...
    public static ServiceError Conflict(string? message = null) => new(ServiceErrorType.Conflict, message ?? "Conflict.");
    ...
    public static ServiceError Conflict<TEntity>(string? message = null) => new(ServiceErrorType.Conflict, message ?? $"{typeof(TEntity).Name} conflict.");
}
