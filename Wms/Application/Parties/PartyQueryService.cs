...
namespace Wms.Application.Parties;

public class PartyQueryService(IDbContextFactory<ApplicationDbContext> dbContextFactory)
{
    public async Task<IReadOnlyDictionary<PartyReference, PartyInfo>> GetManyAsync(
        IEnumerable<PartyReference> references,
        CancellationToken ct = default)
    {
        using var activity = AppTracing.StartActivity("Party GetMany", nameof(PartyQueryService));
