...
namespace Wms.WebApp.Components.Pages.InventoryMovementPages;
...
public partial class Index
{
    ...
    private async Task<GridData<InventoryMovementListItem>> LoadServerDataAsync(
        GridState<InventoryMovementListItem> state,
        CancellationToken cancellationToken)
    {
        ...
        return new GridData<InventoryMovementListItem>
        {
            Items = result.Items,
    ...
    {
        var result = await WarehouseService.ListAsync(new ListQuery { SearchString = searchText, SortBy = "Name", Take = 10 }, ct);
        return result.Items;
    }
    ...
    {
        var result = await StockKeepingUnitService.ListAsync(new ListQuery { SearchString = searchText, SortBy = "Name", Take = 10 }, ct);
        return result.Items;
    }
    ...
    private static string? GetRecorderHref(InventoryMovementListItem item)
    {
    ...
    private static string GetRecorderText(InventoryMovementListItem item) =>
